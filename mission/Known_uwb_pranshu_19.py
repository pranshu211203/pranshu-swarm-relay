...
RELAY_HIERARCHY = [19, 20, 21, 28, 29, 22, 23, 24, 25, 14, 27]
status = ""
course = 0
...
def validate_waypoints():
    ...
    for i, wp in enumerate(data['wp']): 
        ...
        if wp['dist_cm'] < 20 and not i+1 == len(data['wp']):
            ...
            # Don't fail validation — movement code handles short distances gracefully
...
def execute_waypoints(drone):   # edit this up for all heading instances and waypoint updates
   ...
   # Navigate to starting position — chunked to max 200cm per command
    ...
    #--- STEP 2: EXECUTE WAYPOINTS IN SEQUENCE ---
    try: 
...
def main():
    ...
    if drone_id in RELAY_HIERARCHY:
        rank = RELAY_HIERARCHY.index(drone_id)
        stagger_delay = rank * 0.5  # 1 second delay per rank level
    else:
    ...
    # --- FLY TOWARD START DURING STAGGER (instead of just hovering) ---
    if drone_id in RELAY_HIERARCHY:
        rank = RELAY_HIERARCHY.index(drone_id)
...
if __name__ == "__main__":
    ...
    # --- DYNAMIC HIERARCHY DIRECTION ---
    first_wp_x = relay_data['wp'][0]['position_cm']['x'] / 100  # cm to m
    if first_wp_x > 10: # (Area = 20m * 20m, so 10m is the center line)
        RELAY_HIERARCHY.reverse()
        print(f"[RELAY] First WP x={first_wp_x:.1f}m (right side). Hierarchy REVERSED: {RELAY_HIERARCHY}")
    else:
        print(f"[RELAY] First WP x={first_wp_x:.1f}m (left side). Hierarchy unchanged: {RELAY_HIERARCHY}")
